...
class FloatingAnimationHelper(private val context: Context) {
    ...
    fun showFloatingAnimation(
        ...
        // Create the floating view container
        ...
        // Add floating view to parent
        val params = FrameLayout.LayoutParams(
            ViewGroup.LayoutParams.WRAP_CONTENT,
            ViewGroup.LayoutParams.WRAP_CONTENT
        ).apply {
            leftMargin = location[0] + (anchor.width / 2) - 50 // Center approximately
            topMargin = location[1] - 50 // Start above button
        }

        if (parent is FrameLayout) {
            parent.addView(floatingView, params)
        } else {
            // If not FrameLayout, wrap in one
    ...
    private fun animateFloating(
        ...
        // Translation Y animation (move upward)
        ...
        val fadeOut = ObjectAnimator.ofFloat(view, "alpha", 1f, 0f).apply {
            this.duration = duration
            startDelay = duration / 3 // Start fading after 1/3 of animation
        }
